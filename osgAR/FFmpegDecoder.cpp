...
void FFmpegDecoder::findAudioStream()
{
    ...
    {
        if (m_format_context->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO)//CODEC_TYPE_AUDIO)
        {
...
void FFmpegDecoder::findVideoStream()
{
    for (unsigned int i = 0; i < m_format_context->nb_streams; ++i)
    {
        if (m_format_context->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO)//CODEC_TYPE_VIDEO)
        {
